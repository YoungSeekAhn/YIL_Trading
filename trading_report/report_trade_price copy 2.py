...
- 신뢰도(confidence) 내림차순 정렬 기본 적용
- v3/v4 혼용 컬럼명을 자동 매핑(h1_MAPE(%) vs h1_close_MAPE(%), 등)
- 순수 HTML/CSS/JS (외부 의존 X)

사용 예:
    python make_web_report_v4.py --csv ./Report_20251010/Trading_price_20251010.csv --out ./Trading_Report_20251010.html --with-cards
"""
import argparse
import html
import pandas as pd
...
""".format(
            ...
            hz=_safe(hz),
            last_close=_fmt(r.get("last_close")),
            ...
            valid_until=_safe(r.get("valid_until")),
            src=_safe(r.get("source_file")),
        ))
...
<div class="card">
  ...
  <div class="grid">
    ...
    <div><div class="k">유효기한</div><div class="v mono">{valid_until}</div></div>
    <div><div class="k">파일</div><div class="v mono">{src}</div></div>
  </div>
  <div class="conf">
...
""".format(
    ...
    # HTML 템플릿 (중괄호는 그대로, 자리표시는 __PLACEHOLDER__ 사용)
    tpl = """<!doctype html>
<html lang="ko">
<head>
<meta charset="utf-8">
...
def main():
    ...
    ap.add_argument('--csv', required=True, help='Trading_price_<date>.csv 경로')
    ap.add_argument('--out', required=False, help='HTML 출력 경로 (기본: CSV와 같은 폴더)')
    ap.add_argument('--with-cards', action='store_true', help='하단 카드 섹션 표시')
    args = ap.parse_args()
    ...
    html_txt = build_html(df, with_cards=args.with_cards)

    if args.out:
        out_path = Path(args.out)
    else:
        # 기본 파일명: Trading_Report_<date>.html (CSV 파일명 끝의 날짜 토큰 사용)
        try:
            date_token = csv_path.stem.split('_')[-1]
        except Exception:
            date_token = datetime.now().strftime('%Y%m%d')
        out_path = csv_path.parent / f"Trading_Report_{date_token}.html"
